...
const arr1 = Object.keys(fruitsCount);

const arr2 = Object.values(fruitsCount);

let resultString = '';

for (let i = 0; i < arr.length; i++) {
    resultString = resultString + ` ${arr1[i]}: ${arr2[i]}`;
    if (i !== arr.length - 1) {resultString += ';'};
}
...
```js
 const fruit = ['Apple', 'Orange', 'Plum', 'Pineapple'];
let newVariable = fruit[fruit.length - 1];
...
```
```markdown
['Plum', 'Pineapple'] так как индекс 'Apple' будет равен 0, то функция внутри фильтр будет возвращать false и значение 
элемента не будет записано в массив результатов filter => в массив результатов filter запишутся только те элементы 
массиива fruit, индекс которых будет больше 1 (для данного частного случая).
По сути функция внутри filter будет возвращать false для самого искомого элемента и соседних элементов и препятствовать 
записи в массив результатов filter, что и выполнит условие задачи.
```
## Задача/Вопрос 31
```
